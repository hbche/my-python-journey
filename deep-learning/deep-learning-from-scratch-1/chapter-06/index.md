...
上一节主要讲解不同激活函数应该使用合适的初始化权重方法才能取得好的学习效果。根本原因是各层的激活值分布会有适当的广度，从而可以顺利地进行学习。其实，使输出值有合适广度不光只有合适的权重初始值这一种方法，我们可以强制地调整激活值的分布，Batch Normalization 方法就是基于这个想法产生的。

### 6.3.1 Batch Normalization 算法

Batch Normalization 有以下优点：

- 是学习快速进行
- 不那么依赖初始值
- 一直过拟合（降低 Dropout 等的必要性）
