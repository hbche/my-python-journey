...
假设我们想一次投掷多个骰子，并将结果以元组形式返回，则可以重写 roll_dice() 函数
。
...
如果每次都投掷一个骰子，但是每次又要传入 dice=1 这个参数，有没有什么办法能够在不
传这个参数的情况下，默认掷一个骰子吗？当然有，那就是函数的默认参数。
...
可读性极为重要。遗憾的是，包含多个参数的函数调用并不总是代码中最具可读性的部分。
关键字参数可以通过标签附加到函数调用中的参数上来帮助解决这个问题。

按照输入的顺序映射的参数称为位置参数。

如果对前面的 roll_dice()函数一无所知，那么当遇到如下所示这行代码时，我们会怎么理
解？
...
个 5 面骰子还是掷 5 个 6 面骰子？如果还有更多的参数，我们可以想象这是多么让人困
惑。这是位置参数的缺点。

正如 Python 之禅所言：**面对太多可能，不要尝试猜测**。

这样做是不好的，因为这会迫使使用者去猜测参数的含义。可以通过使用关键字参数来消除
歧义。不需要更改函数定义就能使用关键字参数，只需要更改函数调用即可，如下所示：
...
当函数有多个可选参数时，这更有帮助。考虑一下，重写 roll_dice()函数，使掷出的骰子
默认是 6 面的:
...
数的参数列表中已经不再重要，我们只需要使用自己想要的参数，剩下的就不用管了。

甚至可以混合使用位置参数和关键字参数，如下所示：
...
这在很多情况下很有用，特别是当我们不想费心命名位置参数但又想使用许多可选参数时。
唯一的规则是关键字参数必须处在函数调用中位值参数之后。
...
如果熟悉 Java 或 C++等严格类型化的语言，可能习惯于编写重载函数，即具有相同名称但
参数不同的多个函数。通常，支持重载函数的编程语言都提供了一致的接口，同时支持不同
类型的参数。

Python 通常不需要重载函数。通过使用动态类型、鸭子类型和可选参数，便可以编写一个
函数来处理所有输入场景。

如果真的需要重载函数，可以使用单分派泛型函数来创建它们。

## 6.6.可变参数

到目前为止，即使使用可选参数，也仍然必须预测可能传递给函数的参数数量。在大多数情
况下，这是可行的，但有时候，我们根本不知道需要多少个参数。

为了解决这个问题，我们的第一反应可能是将所有参数打包到一个元组或列表中。在某些情
况下，这是可行的，但在其他情况下，当调用函数时，这可能会变得很麻烦。

更好的解决方案是使用任意参数列表，它会自动将多个参数打包到一个可变参数或可变位置
参数中。在掷骰子函数中，我们希望能够掷多个骰子，每个骰子可能有不同面数，如下所示
：
...
在函数内部，可以像往常一样使用这个元组。在这种情况下，不妨使用生成器表达式来掷除
dice 中指定的骰子。
...
以上代码中的两个函数调用列出了我们想要掷的骰子，其中数组表示每个骰子的面数。在第
一个函数调用中，掷了 5 个 6 面骰子。在第二个函数调用中，则掷了 4 个骰子：一个 20
面骰子、一个 6 面骰子、一个 8 面骰子和一个 4 面骰子。

如果想使用递归方法，则可以通过自动将元组解包到函数调用中来填充参数列表，如下所示
：
...
入的。这些参数不能作为位置参数传递值，而只能作为关键字参数。这可以确保以正确的方
式使用特别长或危险的参数列表，而不是将其作为几乎无法阅读的位置参数链对待。
...
以上代码使用了未命名的可变参数\*，以确保参数列表中该可变参数之后的每个参数都只能
...
在这个例子中，参数 dice 仍然具有默认值 1，但现在它是仅位置参数。另一方面，sides
可以作为位置参数或关键字参数使用。
...
dice_cup3 = roll_dice(sides=20)         # dices=1, sides=20
# 位置参数+关键字参数
dice_cup4 = roll_dice(4, sides=20)      # dices=4, sides=20
```

以上调用都是等效的，因为仅位置参数 dice 要么作为第一个参数包含在内，要么完全省略
。如果试图通过关键字参数指定 dice 参数，会报错：

```python
dice_cup4 = roll_dice(dice=4)
...
为了确保关于位置参数和关键字参数的所有内容都清晰，我们将花一点时间使用以下例子（
不可否认的是，这是一个虚构的例子）来进行介绍：
...
下一个参数是 pos_kw，它可以是位置参数或关键字参数，位于任何仅位置参数和斜线(/)标
记（如果有的话）之后。
...
有时候，我们希望在函数内部复用逻辑，但不想创建另一个函数，以免代码变得更加混乱。
在这种情况下，可以在函数中嵌套函数。
...
这个例子将掷骰子的逻辑移到了嵌套函数 roll 中，以便在 roll_dice() 函数的任何地方
调用它。

这样就可以更轻松地维护掷骰子的逻辑，而不会干扰其他代码。如下所示：
...
在生产环境中，我们很少使用嵌套函数来处理这样微不足道的事情。通常，我们会使用嵌套
函数来实现更复杂的逻辑，这些逻辑经常重复使用，特别是当它们在外部函数中的多个位置
使用时。

嵌套函数可以访问其封闭作用域中的变量。但是，如果想在嵌套函数内部重新绑定或改变其
中的任何变量，则需要使用 nonlocal 关键字。
...
在掷骰子示例的基础上，编写一个函数，它返回一个闭包，用于掷出一组特定的骰子，如下
所示：
...
以上代码创建了函数 make_dice_cup()，它有两个参数：sides 和 dice。嵌套函数 roll()
定义在 make_dice_cup()中，它也使用了参数 sides 和 dice。当外部函数返回了该嵌套函
数时，它就变成了一个闭包，因为它使用了 sides 和 dice。
